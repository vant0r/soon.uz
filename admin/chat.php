...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send_message') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            $thread = dbFetchOne('SELECT * FROM chat_threads WHERE id = :id AND status <> :status', ['id'=>$threadId,'status'=>'closed']);
            if (!$thread) throw new RuntimeException('Chat topilmadi yoki yopilgan');
            ...
            dbQuery('UPDATE chat_threads SET admin_id = :admin_id, last_message_at = NOW(), last_message_by = :last_message_by, unread_user_count = unread_user_count + 1, updated_at = NOW() WHERE id = :id', ['admin_id'=>$admin['id'],'last_message_by'=>'admin','id'=>$threadId]);
...
if ($selectedThread) {
    dbQuery('UPDATE chat_threads SET unread_admin_count = 0, updated_at = NOW() WHERE id = :id', ['id'=>$selectedThreadId]);
    dbQuery('UPDATE chat_messages SET is_read = 1, read_at = NOW() WHERE thread_id = :id AND sender_type = :type AND is_read = 0', ['id'=>$selectedThreadId,'type'=>'user']);
}

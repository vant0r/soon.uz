...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($user['status'] !== 'active') {
        ...
        try {
            ...
            dbInsert('chat_messages', ['thread_id'=>$threadId,'sender_type'=>'user','sender_id'=>$userId,'message'=>$text !== '' ? $text : 'Fayl yuborildi','file_path'=>$upload['path'] ?? null,'file_type'=>$upload['type'] ?? null,'file_size'=>$upload['size'] ?? null,'is_read'=>0]);
            dbUpdate('chat_threads', ['last_message_at'=>date('Y-m-d H:i:s'),'last_message_by'=>'user','unread_admin_count'=>'unread_admin_count + 1','status'=>'open'], 'id = :id', ['id'=>$threadId]);
...
dbUpdate('chat_threads',['unread_user_count'=>0],'id=:id',['id'=>$threadId]);
dbQuery('UPDATE chat_messages SET is_read = 1, read_at = NOW() WHERE thread_id = :id AND sender_type = :type', ['id'=>$threadId,'type'=>'admin']);
...
<?php if(!$messages): ?><div style="margin:auto;color:var(--text-muted)">Hali xabarlar yo‘q.</div><?php else: foreach($messages as $msg): ?><div class="bubble <?php echo $msg['sender_type']==='user'?'user':'admin'; ?>"><?php if($msg['message']): ?><div><?php echo nl2br(e($msg['message'])); ?></div><?php endif; ?><?php if($msg['file_path']): ?><?php $ext=strtolower(pathinfo($msg['file_path'],PATHINFO_EXTENSION)); if(in_array($ext,['jpg','jpeg','png','webp','gif'],true)): ?><img src="../<?php echo e($msg['file_path']); ?>" alt="Yuborilgan rasm"><?php else: ?><a href="../<?php echo e($msg['file_path']); ?>" target="_blank" rel="noopener">📎 Faylni ochish</a><?php endif; ?><?php endif; ?><div class="meta"><?php echo e($msg['sender_type']==='admin'?($msg['sender_name']?:'Admin'):'Siz'); ?> · <?php echo e(date('d.m.Y H:i',strtotime($msg['created_at']))); ?></div></div><?php endforeach; endif; ?>
</div>
...
<?php include __DIR__ . '/includes/footer.php'; ?><script src="../assets/js/main.js"></script><script>const messages=document.getElementById('messages');if(messages)messages.scrollTop=messages.scrollHeight;</script>
</body></html>
